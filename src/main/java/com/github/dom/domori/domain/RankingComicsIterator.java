...
public class RankingComicsIterator implements Iterator<List<Comic>> {

    private static final Logger LOGGER = LoggerFactory.getLogger(RankingComicsIterator.class);
    ...
    public RankingComicsIterator(RankingTerms term, DelayController delay) {
        this.term = term;
        this.startUrl = String.format(EndPoints.COMICS_RANKING_FORMAT, term.getTerm());
    ...
    public List<Comic> next() {
        try {
            ...
            String url = String.format(EndPoints.CRAWL_COMICS_API_FORMAT, currentPage, term.getTerm());
            Connection.Response response = Jsoup.connect(url).userAgent(Constants.USER_AGENT).referrer(startUrl).execute();

            String json = response.body();

            JsonParser parser = new JsonParser();
            return parser.parseRankingComics(json);
